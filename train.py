#!/usr/bin/env python3
# train.py - training script compatible with PSINetRDBUNet that returns dict outputs
# - Supports: seed, binary_output, lowfreq & TV losses, region weighting, resume, amp
# - Handles model returning dict {'denoised', 'res_low', ...} or raw tensor
...
def gaussian_blur(x, sigma):
    ...
    kernel2d = kernel2d.unsqueeze(0).unsqueeze(0)  # 1,1,S,S
    C = x.shape[1]
...
# Train / val loops (handle dict outputs)
def train_epoch(model, loader, optimizer, device, loss_params, scaler, use_amp=True, vgg_extractor=None, args=None):
    ...
    for i, (noisy, clean) in enumerate(loader):
        ...
        with autocast(enabled=use_amp):
            ...
            # prepare clean gray (if clean is RGB -> convert)
            if clean.shape[1] == 3:
                clean_gray = 0.2989*clean[:,0:1,:,:] + 0.5870*clean[:,1:2,:,:] + 0.1140*clean[:,2:3,:,:]
            else:
                clean_gray = clean

            # optional region weighting (lower half emphasis)
            mask = None
            ...
            # low-frequency loss: compare blurred predictions to blurred GT
    ...
    with torch.no_grad():
        for i, (noisy, clean) in enumerate(loader):
            ...
            if vis_dir and i == 0:
                ...
                noisy_vis = noisy[:max_vis].cpu()
                pred_vis = pred[:max_vis].cpu().repeat(1,3,1,1)
                clean_vis = clean[:max_vis].cpu()
...
def main():
    ...
    # extras
    parser.add_argument("--binary_output", action="store_true", help="Train to output binary/black-white")
    ...
    # datasets (support StripeDataset updated to force_gray/binarize if desired)
    # If you want dataset to perform force_gray/binarize, edit stripe_dataset or pass flags there.
